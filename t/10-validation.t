...
# --- _readBlockData(): a -1 return croaks instead of over-reading (V2 guard) ---

# Pre-V2, on a -1 return the XSUB fell through to sv_setpvn(output, buf, ret)
# with ret == -1 - a (STRLEN)-1 over-read that could segfault the interpreter.
# fd -1 forces the underlying ioctl to fail (-1) with no bus or device touched,
# so the croak guard is exercised HW-free.
{
    my $buf = "\0" x 32;
    eval { RPi::I2C::_readBlockData(-1, 0, $buf) };
    like $@, qr/_readBlockData\(\) has invalid return/,
        "_readBlockData(-1, 0, \$buf) croaks on a -1 return instead of over-reading";
}
